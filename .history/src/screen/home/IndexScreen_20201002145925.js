...
class Home extends Component {
    ...
    renderUser() {
        ...
        return users.map((data, i) => {
            let mb = i === users.length - 1 ? 0 : '5%';
            console.log(mb)
            return (
                <Card key={i} style={{ height: 100, marginBottom: mb, display: 'flex', justifyContent: 'center', alignItems: 'center', paddingBottom: 16 }}>{data.name}</Card>
            );
        })
    }
            ...
            <div>
                ...
                <div style={body}>
                    <div style={bodyContext}>
                        ...
                        <div style={{height: '70%', padding: '10px 20%' }}>
                            <div style={{ height: '100%', overflow: 'auto', padding: '10px 20%' }}>
                                {this.renderUser()}
                            </div>
                        </div>
                    </div>
