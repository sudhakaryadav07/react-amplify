...
class Home extends Component {
    ...
    renderUser() {
        ...
        return users.map((data, i) => {
            let mb = i === users.length - 1 ? '2%' : 0;
            let mt = i === 0 ? '2%' : 0;
            return (
                <Card key={i} style={{ height: 100, margin: `${mt} 0px ${mb} 0px`, display: 'flex', justifyContent: 'center', alignItems: 'center', paddingBottom: 16 }}>{data.name}</Card>
            );
        })
    }
