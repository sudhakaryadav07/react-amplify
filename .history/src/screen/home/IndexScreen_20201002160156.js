...
class Home extends Component {
    ...
    renderUser() {
        ...
        return users.map((data, i) => {
            let mt = i === 0 ? '2%' : 0;
            return (
                <Card key={i} style={{ height: 140, margin: `${mt} 0px 2% 0px`, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <Grid container sm={12} style={{ height: '100%' }}>
                        <Grid item container item sm={4}>
                            <Grid item sm={12} style={{ height: '80%', backgroundColor: 'red' }}>
                                <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <img alt="" style={{ height: '86%', width: '47%' }} src={require(`../../icons/user.jpg`)} />
                                </div>
                            </Grid>
