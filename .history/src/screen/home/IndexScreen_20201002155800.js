...
class Home extends Component {
    ...
    renderUser() {
        ...
        return users.map((data, i) => {
            let mt = i === 0 ? '2%' : 0;
            return (
                <Card key={i} style={{ height: 100, margin: `${mt} 0px 2% 0px`, display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                    <Grid container sm={12} style={{ height: '100%' }}>
                        <Grid item container item sm={4}>
                            <Grid item sm={12} style={{ height: '85%', backgroundColor: 'red' }}>
                                <img alt="" style={{ height: '10%', width: '5%' }} src={require(`../../icons/user.png`)} />
                            </Grid>
                            <Grid item sm={12} style={{ height: '15%' }}>
