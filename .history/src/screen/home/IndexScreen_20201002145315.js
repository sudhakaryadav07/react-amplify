...
class Home extends Component {
    ...
    renderUser() {
        ...
        return users.map((data, i) => {
            let mb = i === users[users.length - 1] ? 0 : '5%';
            console.log(mb)
            return (
                <Card  key={i} style={{ height: 100, margin: '0% 20%', marginBottom: mb, display: 'flex', justifyContent: 'center', alignItems: 'center', paddingBottom: 16 }}>{data.name}</Card>
            );
        })
    }
